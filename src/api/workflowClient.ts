...
export async function loadWorkflowCatalog(): Promise<WorkflowCatalog> {
  const { response } = await fetchLgs("/api/v1/workflows");
  if (!response.ok) await throwForResponse(response, "Could not load Local Gen Studio workflows");
  ...
  if (
    ...
    throw new RemoteApiError("Local Gen Studio returned an invalid workflow catalog.");
  }
...
export async function submitWorkflowGeneration(
  ...
  onStatus?.("submitting");
  const submittedCall = await fetchLgs("/api/v1/generate/workflow", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(request),
  });
  const response = submittedCall.response;
  if (!response.ok) await throwForResponse(response, "Workflow generation was rejected");

  const submitted: unknown = await response.json();
  if (typeof submitted !== "object" || submitted === null || !("jobId" in submitted)) {
    throw new RemoteApiError("Local Gen Studio returned an invalid generation response.");
  }
  ...
  while (true) {
    ...
    if (!statusResponse.ok) await throwForResponse(statusResponse, "Could not read generation status");
